...
type mockInserter struct {
	...
	insertedData    []map[string][]labeledSamplesInfo
...
func (m *mockInserter) InsertNewData(rows map[string][]labeledSamplesInfo) (uint64, error) {
	return m.InsertData(rows)
}

func (m *mockInserter) InsertData(rows map[string][]labeledSamplesInfo) (uint64, error) {
	for _, v := range rows {
		for i, si := range v {
